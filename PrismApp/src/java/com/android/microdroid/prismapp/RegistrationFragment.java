...
public class RegistrationFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		((MainActivity) getActivity()).getModel().prepRegistration();
		view.setOnTouchListener(new View.OnTouchListener() {
                   public boolean onTouch(View v, MotionEvent event) {
                      if(event.getAction() == MotionEvent.ACTION_DOWN){        
                        ...
                        float x = event.getX();
                        float y = event.getY();
                        //Log.i("i","Reg Clicked x: "+x+" y: "+y+"\n");
                        if (y>1500){
		            ((MainActivity) getActivity()).getModel().registerAlias();
		            ((MainActivity) getActivity()).showMain();              
		        }
		        else{
		            ((MainActivity) getActivity()).getModel().registerKeyInput((int)x, (int)y);
